    ...
    describe("Harvest contract", function() {
        ...
        it("Sell ERC721 to Harvestooor", async function() {
            ...
            await userHarvestooor.sellERC721(WHATTY_CLUB_ADDRESS, WHATTY_CLUB_TOKEN_ID);
        ...
        it("Sell too many ERC1155 tokens", async function() {
            ...
            const userHarvestooor = await harvestooorContract.connect(signer);
            await truffleAssert.reverts(userHarvestooor.sellERC1155(APEIN_FINANCE_SHOP_ADDRESS, WEAK_HANDS_TOKEN_ID, 30), "Must sell 25 or less NFTs in a single transaction");
        });
        ...
        it("Sell ERC1155 tokens", async function() {
            ...
            const userHarvestooor = await harvestooorContract.connect(signer);
            await userHarvestooor.sellERC1155(APEIN_FINANCE_SHOP_ADDRESS, WEAK_HANDS_TOKEN_ID, 3);
        ...
        it("Batch sell too many ERC1155 tokens", async function() {
            ...
            const userHarvestooor = await harvestooorContract.connect(signer);
            await truffleAssert.reverts(userHarvestooor.sellBatchERC1155(APEIN_FINANCE_SHOP_ADDRESS, [4,WEAK_HANDS_TOKEN_ID], [1,30]), "Must sell 25 or less NFTs of a single ID per transaction");
        });
        ...
        it("Batch sell ERC1155 tokens", async function() {
            ...
            const userHarvestooor = await harvestooorContract.connect(signer);
            await userHarvestooor.sellBatchERC1155(APEIN_FINANCE_SHOP_ADDRESS, [4,WEAK_HANDS_TOKEN_ID], [1,20])
            const afterBal = await fraxContract.balanceOf(signer.address);
